"""
Hiperideal Scraper - API GraphQL HTTP Pura (sem Playwright)
...
seguindo o padrão dos outros scrapers.
...
class HiperidealScraper(BaseAPIScraper):
    """
    Scraper para Hiperideal via API GraphQL VTEX.
    
    Usa requisições HTTP diretas - sem necessidade de browser.
    
    API Endpoint:
        GET https://www.hiperideal.com.br/_v/segment/graphql/v1
        
    A VTEX usa persisted queries com variáveis em Base64.
    
    Configurações do Hiperideal:
    - hideUnavailableItems: false (mostra indisponíveis)
    - skusFilter: "ALL"
    - 20 produtos por página
    """
    ...
    def __init__(self, config=None):
        ...
        """
        config = config or HIPERIDEAL_CONFIG
