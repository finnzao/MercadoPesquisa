"""
Scraper específico para Sam's Club Brasil - API VTEX GraphQL
https://www.samsclub.com.br

Este scraper usa a API GraphQL do Sam's Club (plataforma VTEX) para buscar produtos.
Similar ao GBarbosa, usa persisted queries com variáveis codificadas em base64.
...
class SamsClubScraper(LoggerMixin):
    """
    Scraper para Sam's Club Brasil usando API GraphQL VTEX.
    ...
    Similar ao GBarbosa, o fluxo correto é:
    ...
    PRODUCTS_PER_PAGE = 24  # Sam's Club retorna até 24 por página

    def __init__(self, config: Optional[MarketConfig] = None):
        """Inicializa o scraper."""
        self.config = config or SAMSCLUB_CONFIG
    ...
    async def search(
        ...
        """
        Executa busca no Sam's Club usando a API GraphQL.
    ...
    def _build_search_page_url(self, query: str) -> str:
        ...
        Formato: https://www.samsclub.com.br/{query}?_q={query}&map=ft
    ...
    def _build_graphql_variables(
        ...
        Baseado na estrutura observada no HAR do Sam's Club.
        """
    ...
    async def _extract_from_html(
        ...
        try:
            ...
            # Seletores para cards de produto do Sam's Club
            selectors = [
    ...
    def _parse_product(
        ...
        # Tenta obter preço do primeiro SKU/Seller
        skus = item.get("items", [])
        for sku in skus:
...
async def search_samsclub(
    query: str,
    cep: Optional[str] = None,
    max_pages: int = 1,
) -> ScraperResult:
    """
    Função de conveniência para busca rápida no Sam's Club.
    ...
    Exemplo:
        result = await search_samsclub("arroz 5kg")
        for product in result.products:
            print(f"{product.title}: {product.price_raw}")
    """
    scraper = SamsClubScraper()
    ...
    async def test():
        print("=" * 80)
        print("TESTE DO SCRAPER SAM'S CLUB (API GraphQL)")
        print("=" * 80)
