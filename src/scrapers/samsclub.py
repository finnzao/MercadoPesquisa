"""
Sam's Club Scraper - VTEX REST API Otimizado
...
class SamsClubScraper(VTEXOptimizedScraper):
    """
    Scraper para Sam's Club via VTEX REST API.
    ...
    def __init__(self, config: Optional[MarketConfig] = None):
        ...
        Args:
            config: Configuração do mercado (opcional, usa SAMSCLUB_CONFIG)
        """
        super().__init__(config or SAMSCLUB_CONFIG)
    
    def _get_additional_headers(self) -> Dict[str, str]:
        """
        Headers adicionais específicos do Sam's Club.
