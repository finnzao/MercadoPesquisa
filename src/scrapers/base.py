"""
Classe base CORRIGIDA para scrapers de mercados.
Com configurações melhoradas para evitar detecção de bot.
...
class BaseScraper(ABC, LoggerMixin):
    ...
    async def _check_for_blocks(self, page: Page) -> None:
        ...
        for indicator, block_type in block_indicators:
            if indicator in content_lower:
                ...
                if not self._is_false_positive_block(content_lower, indicator):
                    raise BlockedError(
                        f"Detectado possível bloqueio: {indicator}",
                        market_id=self.market_id,
                        block_type=block_type,
                    )
    
    def _is_false_positive_block(self, content: str, indicator: str) -> bool:
        """Verifica se é falso positivo de bloqueio."""
        # Alguns sites mencionam essas palavras em contextos normais
        false_positive_contexts = [
            "produto robô",  # Produto com nome robô
            "robô aspirador",  # Produto robô aspirador
            "vacuum robot",
        ]
    ...
    async def _init_browser(self) -> None:
        ...
        # Usa Chromium padrão (não headless shell) para parecer mais real
        self._browser = await self._playwright.chromium.launch(
            headless=False,  # Pode mudar para False para debug
            args=[
