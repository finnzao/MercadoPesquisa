"""
Scraper específico para Mercantil Atacado - API VTEX IO GraphQL
https://www.mercantilatacado.com.br

Este scraper usa a API GraphQL do Mercantil Atacado (plataforma VTEX IO).
Usa exatamente o mesmo padrão do Sam's Club e GBarbosa:
- Persisted queries com variáveis codificadas em Base64
- Mesmo SHA256 hash da query productSearchV3

Endpoint principal:
  GET https://www.mercantilatacado.com.br/_v/segment/graphql/v1?operationName=productSearchV3&...
...
# CONFIGURAÇÃO DO MERCANTIL ATACADO
...
class MercantilAtacadoScraper(LoggerMixin):
    """
    Scraper para Mercantil Atacado usando API GraphQL VTEX IO.

    Usa o mesmo padrão do Sam's Club e GBarbosa:
    - Endpoint: /_v/segment/graphql/v1
    - Operation: productSearchV3
    - Persisted queries com variáveis em Base64

    O fluxo é:
    1. Navegar para a URL de busca primeiro (estabelece contexto)
    2. Fazer requisição GraphQL de dentro do navegador

    Uso:
        scraper = MercantilAtacadoScraper()
    ...
    # Configurações da API (idênticas ao Sam's Club)
    ...
    def __init__(self, config: Optional[MarketConfig] = None):
        ...
        self.config = config or MERCANTIL_CONFIG
    ...
    async def search(
        ...
        """
        Executa busca no Mercantil Atacado usando a API GraphQL.
        ...
        try:
            ...
            # PASSO 2: Navega para a URL de BUSCA (estabelece contexto)
            ...
            for page_num in range(1, max_pages + 1):
                ...
                if api_data and not api_data.get("error"):
                    ...
                    products = self._parse_graphql_response(
                        api_data, query, cep)
    ...
    def _build_search_page_url(self, query: str) -> str:
        ...
        Formato: https://www.mercantilatacado.com.br/{query}?_q={query}&map=ft
        """
    ...
    def _build_graphql_variables(
        ...
        Nota: Mercantil usa skusFilter: "FIRST_AVAILABLE" em vez de "ALL"
        """
        query_clean = query.strip()

        return {
            "hideUnavailableItems": True,
            "skusFilter": "FIRST_AVAILABLE",  # Diferente do Sam's Club
    ...
    async def extract_products(
        ...
        """Compatibilidade com interface BaseScraper."""
        api_data = await self._fetch_graphql(page, search_query, 0, 23)
        if api_data and not api_data.get("error"):
...
async def search_mercantil(
    query: str,
    cep: Optional[str] = None,
    max_pages: int = 1,
) -> ScraperResult:
    """
    Função de conveniência para busca rápida no Mercantil Atacado.
    ...
    Exemplo:
        result = await search_mercantil("arroz 5kg")
        for product in result.products:
            print(f"{product.title}: {product.price_raw}")
    """
    scraper = MercantilAtacadoScraper()
